...
float4 PS(VS_OUT inData) : SV_Target
{
	...
	float4 picColor = float4(1, 1, 1, 0.01);

	const int cellSize = 2;
	//とうめいー
	if (true) {
		if ((inData.pos.x / cellSize + inData.pos.y / cellSize) % 2 == 0)
			return diffuse * shade + diffuse * ambient + speculer;
		else
			return picColor * diffuse * shade + picColor * diffuse * ambient + picColor * speculer;
	}
